...
(function initInstallGuide(){
  ...
  if(isIOS){
    content.innerHTML=`<div class="guide-title">🍎 للـ iPhone و iPad</div><ol><li>افتح الموقع من <strong>Safari</strong>.</li><li>اضغط زر <strong>المشاركة ⬆️</strong>.</li><li>اختر <strong>إضافة إلى الشاشة الرئيسية</strong>.</li><li>اضغط <strong>إضافة</strong> لتظهر أيقونة أمازون هايبر ماركت.</li></ol>`;
  }else if(isAndroid){
    content.innerHTML=`<div class="guide-title">🤖 لأجهزة Android</div><ol><li>افتح الموقع في <strong>Chrome</strong>.</li><li>اضغط <strong>⋮</strong>.</li><li>اختر <strong>تثبيت التطبيق</strong> أو <strong>إضافة إلى الشاشة الرئيسية</strong>.</li><li>اضغط <strong>تثبيت</strong> لتظهر الأيقونة.</li></ol>`;
  }else{
    content.innerHTML=`<div class="guide-title">💻 للكمبيوتر</div><ol><li>افتح الموقع في Chrome أو Edge.</li><li>من قائمة المتصفح اختر <strong>تثبيت التطبيق</strong>.</li><li>بعد التثبيت يمكنك إنشاء اختصار على سطح المكتب.</li></ol>`;
  }

  const show=()=>{ modal.classList.add("show"); modal.setAttribute("aria-hidden","false"); };
  const hide=()=>{ modal.classList.remove("show"); modal.setAttribute("aria-hidden","true"); localStorage.setItem("ah_install_guide_seen","1"); };
  open?.addEventListener("click",show);
  ok?.addEventListener("click",hide);
  close?.addEventListener("click",hide);
  modal.addEventListener("click",e=>{if(e.target===modal) hide();});
  if(!localStorage.getItem("ah_install_guide_seen")) setTimeout(show,650);
})();
