...
function renderCategorySubnav(){
  ...
  const subs=getCategorySubcategories(selectedCategory).filter(c=>c.visible!==false&&c.hidden!==true&&c.active!==false);
  nav.innerHTML=`
...
function renderCategories(){
  ...
  if(!el) return;
  const mainCategories = categories.filter(c => !(c.parentId ?? c.parent_id ?? c.parent) && c.visible !== false && c.hidden !== true && c.active !== false);
