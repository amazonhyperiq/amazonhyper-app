...
function renderProducts(){
  ...
  // التعامل مع + و− عبر event delegation حتى تعمل مع أي نوع من المعرّفات،
  // بما فيها UUID القادمة من Supabase.
  if(!el.dataset.qtyEventsBound){
    el.addEventListener("click", (event) => {
      const button = event.target.closest("button[data-product-id]");
      if(!button || !el.contains(button)) return;
      const id = button.dataset.productId;
      if(button.classList.contains("product-qty-plus")){
        changeQty(id, 1);
      }else if(button.classList.contains("product-qty-minus")){
        changeQty(id, -1);
      }
    });
    el.dataset.qtyEventsBound = "1";
  }
        ...
        return `
          ...
          <article class="product">
            ...
            <div class="qty">
              ...
              <button
                type="button"
                class="product-qty-plus"
                data-product-id="${String(p.id).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}"
                aria-label="زيادة"
              >
                +
              </button>
...
function renderHeroBanner(){
  ...
  // دعم إعدادات البنر المحفوظة من لوحة الإدارة، مع إبقاء البنر ظاهراً
  // حتى عند استخدام صورة أو نص فقط.
  const heroImage = String(heroBanner.heroImage || heroBanner.image || "").trim();
  const heroType = String(heroBanner.heroType || "image-text").trim();
  if(heroImage){
    banner.style.backgroundImage = `linear-gradient(90deg, rgba(4,45,23,.88), rgba(4,45,23,.38)), url("${heroImage.replace(/"/g, '\\"')}")`;
    banner.style.backgroundSize = "cover";
    banner.style.backgroundPosition = "center";
  }else{
    banner.style.backgroundImage = "";
  }
  banner.classList.toggle("hero-image-only", heroType === "image");
  banner.classList.toggle("hero-text-only", heroType === "text");
