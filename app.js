...
function renderSiteContent(){
  ...
  section.className = "section site-info-compact";
  section.style.cssText =
    "background:linear-gradient(135deg,#f7fff8,#eef8f1);" +
    "border:1px solid #d7e9dc;margin-bottom:12px;padding:13px 14px;";

  const head = document.createElement("div");
  head.className = "site-info-compact-head";
  head.style.cssText =
    "display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;";

  const intro = document.createElement("div");
  intro.style.cssText = "min-width:0;flex:1 1 280px;";

  const title = document.createElement("strong");
  title.textContent = c.welcomeTitle || "أهلاً وسهلاً بكم في أمازون هايبر ماركت 🛒";
  title.style.cssText =
    "display:block;color:#064f2b;font-size:17px;line-height:1.45;font-weight:900;";
  intro.appendChild(title);

  const welcome = document.createElement("p");
  welcome.textContent = c.welcomeText || "";
  welcome.style.cssText =
    "margin:3px 0 0;color:#617067;font-size:10px;line-height:1.65;";
  ...
  function makeToggle(label, target){
    ...
    btn.style.cssText =
      "border:1px solid #bfe1cc;background:#fff;color:#086b39;border-radius:999px;" +
      "padding:8px 12px;font:800 11px inherit;cursor:pointer;box-shadow:0 5px 14px rgba(5,83,39,.08);";
  ...
  actions.appendChild(head.appendChild ? document.createElement("span") : null);
  actions.lastChild && actions.lastChild.remove();

  head.appendChild(actions);
  section.appendChild(head);

  const detailsWrap = document.createElement("div");
  detailsWrap.style.cssText =
    "display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px;margin-top:9px;";

  const howBox = document.createElement("div");
  howBox.className = "site-info-box site-info-collapsible";
  howBox.style.cssText =
    "background:#fff;border:1px solid #dceae1;border-radius:13px;padding:10px 11px;";
  const howContent = document.createElement("div");
  howContent.hidden = true;
  howContent.style.cssText =
    "margin-top:9px;white-space:pre-line;line-height:1.85;color:#37413b;font-size:11px;";
  howContent.textContent = c.howToUse || "";
  howBox.appendChild(howContent);

  const infoBox = document.createElement("div");
  infoBox.className = "site-info-box site-info-collapsible";
  infoBox.style.cssText =
    "background:#fff;border:1px solid #dceae1;border-radius:13px;padding:10px 11px;";
  const infoContent = document.createElement("div");
  infoContent.hidden = true;
  infoContent.style.cssText =
    "margin-top:9px;color:#37413b;font-size:11px;line-height:1.85;";

  const phone = document.createElement("p");
  phone.style.margin = "0 0 4px";
  phone.textContent = "📞 هاتف التوصيل: " + (c.deliveryPhone || "");
  infoContent.appendChild(phone);

  const address = document.createElement("p");
  address.style.margin = "4px 0";
  address.textContent = "📍 العنوان: " + (c.storeAddress || "");
  infoContent.appendChild(address);

  const notes = document.createElement("p");
  notes.style.margin = "4px 0";
  notes.textContent = "📝 الملاحظات: " + (c.notesInfo || "");
  infoContent.appendChild(notes);

  const delivery = document.createElement("p");
  delivery.style.cssText = "margin:4px 0;font-weight:800;color:#8b5b00;";
  delivery.textContent = "⚠️ " + (c.deliveryNote || "");
  infoContent.appendChild(delivery);

  howBox.insertBefore(makeToggle("📌 طريقة استخدام الموقع", howContent), howContent);
  infoBox.insertBefore(makeToggle("🚚 معلومات التوصيل", infoContent), infoContent);

  detailsWrap.appendChild(howBox);
  detailsWrap.appendChild(infoBox);
