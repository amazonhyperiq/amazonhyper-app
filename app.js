...
function renderCategories(){
  ...
  /*
     تقسيم الأقسام إلى صفحات ثابتة: 5 أعمدة × 4 صفوف.
     هذا يحافظ على ترتيب الأقسام كما هو محفوظ في لوحة التحكم،
     ويجعل التمرير أفقيًا بين الصفحات فقط بدل تغيير ترتيب العرض.
  */
  const pageSize = 20;
  const pages = [];
...
function renderProducts(){
  ...
  const list=query?products.filter(p=>String(p.name||"").toLowerCase().includes(query)):products.filter(p=>String(p.cat)===String(selectedCategory));
  ...
  }).join("")||`<div style="grid-column:1/-1;text-align:center;padding:30px;color:#6b756f;font-weight:800">${productSearchQuery?"لا توجد مواد مطابقة للبحث حالياً.":"لا توجد مواد في هذا القسم حالياً."}</div>`;
}
...
function renderCartModal(){
    ...
    html += `
      <div class="cart-modal-item">
        <div class="cart-modal-item-info">
          <strong>${p.name || "مادة"}</strong>
          <span>${money(Number(p.price || 0))} / ${unit}</span>
          <b>${money(subtotal)}</b>
        </div>
        <div class="cart-modal-item-controls">
          <button type="button" class="cart-modal-plus" data-cart-id="${String(id).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}" aria-label="زيادة">+</button>
          <span>${qtyText}</span>
          <button type="button" class="cart-modal-minus" data-cart-id="${String(id).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}" aria-label="نقصان">−</button>
          <button type="button" class="cart-modal-remove" data-cart-id="${String(id).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}" aria-label="حذف">حذف</button>
        </div>
      </div>`;
  }

  const deliveryFee = getCurrentDeliveryInfo().deliveryFee || 0;
  const total = productsTotal + deliveryFee;
  ...
  if(itemsEl){
    ...
    // استخدام event delegation يمنع مشاكل معرفات المنتجات (خصوصاً UUID)
    // التي كانت تكسر أزرار السلة عند إدراجها داخل onclick.
...
let heroBanner={enabled:true,kicker:"🛒 أمازون هايبر ماركت",title:"كل احتياجات بيتك",strongTitle:"بخطوات أسهل وأسرع",text:"اختَر القسم، أضف احتياجاتك إلى السلة، ونحن نهتم بالباقي 🚚",buttonText:"ابدأ التسوق الآن ←",buttonAction:"#categories",heroImage:"icons/categories/vegetables.jpg",art:"🛒"};
let heroBanners=[];let heroIndex=0;let heroTimer=null;
function getHeroSlides(){
  const fallback=[
    {enabled:true,kicker:"🥬 طازج يوميًا",title:"خضار وفواكه طازجة",strongTitle:"بجودة تليق ببيتك",text:"تسوّق احتياجاتك بسهولة ونوصلها لك داخل بغداد.",buttonText:"تسوّق الآن ←",buttonAction:"#categories",heroImage:"icons/categories/vegetables.jpg"},
    {enabled:true,kicker:"🥩 اختيارات يومية",title:"لحوم طازجة",strongTitle:"مختارة بعناية",text:"اختَر ما تحتاجه من الأقسام ووفر وقتك مع التسوق أونلاين.",buttonText:"تصفح اللحوم ←",buttonAction:"#categories",heroImage:"icons/categories/meat.jpg"},
    {enabled:true,kicker:"🛒 مواد أساسية",title:"كل احتياجات البيت",strongTitle:"في مكان واحد",text:"مواد غذائية ومشروبات ومنتجات منزلية بأسعار مناسبة.",buttonText:"ابدأ التسوق ←",buttonAction:"#categories",heroImage:"icons/categories/food.jpg"}
  ];
...
function renderHeroBanner(){
 ...
 track.innerHTML=slides.map(h=>{const img=String(h.heroImage||h.image||"").trim();return`<div class="hero-slide">${img?`<div class="hero-bg" style="background-image:url(&quot;${escapeHtml(img)}&quot;)"></div>`:""}<div class="hero-media">${img?`<img src="${escapeHtml(img)}" alt="" loading="lazy">`:`<div class="hero-art">${escapeHtml(h.art||"🛒")}</div>`}</div><div class="hero-copy"><span class="hero-kicker">${escapeHtml(h.kicker||"")}</span><h1>${escapeHtml(h.title||"")}${h.strongTitle?`<br><strong>${escapeHtml(h.strongTitle)}</strong>`:""}</h1><p>${escapeHtml(h.text||"")}</p><button type="button" class="hero-cta" data-hero-action="${escapeHtml(h.buttonAction||"#categories")}">${escapeHtml(h.buttonText||"ابدأ التسوق الآن ←")}</button></div></div>`;}).join("");
...
function renderSiteContent(){
  ...
  grid.className = "site-welcome-grid";

  const howBox =
    document.createElement("div");

  howBox.className = "site-welcome-box";
  ...
  infoBox.className = "site-welcome-box";
...
function initProductSearch(){
  ...
  if(mobileInput){
    mobileInput.addEventListener("input",()=>{
      productSearchQuery=String(mobileInput.value||"").trim();
      if(input) input.value=mobileInput.value;
      const title=document.getElementById("productsTitle");
      if(title) title.textContent=productSearchQuery?"نتائج البحث":(categories.find(c=>String(c.id)===String(selectedCategory))?.name||"المواد");
      updateProductSearchUI();
      renderProducts();
    });
  }
