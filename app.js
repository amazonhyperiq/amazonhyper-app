    ...
    el.addEventListener("click",event=>{
      ...
      if(qty&&el.contains(qty)){
        ...
        if(qty.classList.contains("product-qty-plus")||qty.classList.contains("product-quick")){
          ...
          if(targetProduct && productVariants(targetProduct).length && !activeVariant(targetProduct)) openProductModal(id);
          else changeQty(id,1);
...
function initProductModal(){
  ...
  document.getElementById("productModalAdd")?.addEventListener("click",()=>{
    ...
    if(!(Number(cart.get(selectedProductForModal.id)||0)>0)) changeQty(selectedProductForModal.id,1);
    ...
    html += `
      <div class="cart-modal-item">
        <div class="cart-modal-item-info">
          ...
          ${activeVariant(p)?`<span class="cart-variant-name">النوع: ${escapeHtml(activeVariant(p).name)}</span>`:""}
