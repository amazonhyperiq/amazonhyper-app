...
function initOrderSchedule(){
  ...
  const dateInput = document.getElementById("scheduledDate");
  const timeSelect = document.getElementById("scheduledTime");
  if(!enabled || !fields || !dateInput || !timeSelect) return;
...
function getScheduledOrderInfo(){
  ...
  // الجدولة اختيارية: بدون تفعيلها لا نرسل موعدًا إلى قاعدة البيانات.
  if(!enabled){
    return {valid:true, enabled:false, dateValue:"", timeValue:"", scheduledAt:null, display:""};
  }

  if(!dateValue || !timeValue){
    return {valid:false, enabled:true, dateValue, timeValue, scheduledAt:null, display:""};
  }
...
if(whatsappBtn){
  ...
  whatsappBtn.addEventListener(
    ...
    async () => {
      ...
      if(!schedule.valid){
        alert(
          "فعّلت جدولة الطلب، لذلك يرجى تحديد تاريخ ووقت التوصيل."
        );
      ...
      lines.push(
        ...
        `العنوان: ${address}`,
        ...(schedule.enabled ? [`موعد التوصيل: ${schedule.display}`] : ["موعد التوصيل: أقرب وقت متاح"]),
...
/* =====================================================
   نافذة السلة — فتح وعرض المحتويات
===================================================== */
...
function renderCartModal(){
  ...
  if(!cart || cart.size === 0){
    box.innerHTML = '<div class="cart-empty">السلة فارغة حالياً.</div>';
  }else{
    box.innerHTML = [...cart.entries()].map(([id,q]) => {
      const p = products.find(x => String(x.id) === String(id));
      if(!p) return "";
      const quantity = Number(q) || 0;
      const itemTotal = (Number(p.price) || 0) * quantity;
      return `
        <div class="cart-modal-item">
          <div class="cart-modal-item-info">
            <strong>${String(p.name || "مادة")}</strong>
            <small>${quantity} ${String(p.unit || "")}</small>
            <b>${money(itemTotal)}</b>
          </div>
          <div class="cart-modal-qty">
            <button type="button" onclick="changeQty(${JSON.stringify(p.id)}, 1)">+</button>
            <span>${quantity}</span>
            <button type="button" onclick="changeQty(${JSON.stringify(p.id)}, -1)">−</button>
          </div>
        </div>
  ...
  for(const [id,q] of cart){
    const p = products.find(x => String(x.id) === String(id));
    if(p) productsTotal += (Number(p.price) || 0) * (Number(q) || 0);
  }
  const deliveryInfo = typeof getCurrentDeliveryInfo === "function"
    ? getCurrentDeliveryInfo()
    : {deliveryFee:0};
...
document.addEventListener("DOMContentLoaded", () => {
  document.getElementById("cartOpenBtn")?.addEventListener("click", openCartModal);
  document.getElementById("cartCloseBtn")?.addEventListener("click", closeCartModal);
  document.getElementById("cartContinueBtn")?.addEventListener("click", closeCartModal);
  document.getElementById("cartCheckoutBtn")?.addEventListener("click", () => {
    closeCartModal();
    document.getElementById("whatsappBtn")?.scrollIntoView({behavior:"smooth", block:"center"});
  });
  document.getElementById("cartModal")?.addEventListener("click", (event) => {
    if(event.target?.id === "cartModal") closeCartModal();
  });
});
