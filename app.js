      ...
      categories =
        saved.categories.map(c => ({
          ...
          subcategories:
            ...
            Array.isArray(c.tabs) ? c.tabs : []

        }));

      // المصدر المعتمد لعلاقة الرئيسي/الفرعي هو parentId فقط.
      // نعيد بناء subcategories من parentId حتى لا يبقى فرع قديم ظاهراً بعد تحويله إلى رئيسي أو نقله.
      const categoryById = new Map(categories.map(c => [String(c.id), c]));
      categories.forEach(c => { c.subcategories = []; });
      categories.forEach(child => {
        if (child.parentId == null) return;
        const parent = categoryById.get(String(child.parentId));
        if (!parent || parent === child) return;
        parent.subcategories.push({id: child.id, name: child.name, image: child.image || "", icon: child.icon || "▦"});
      });
...
function getCategorySubcategories(categoryId){
  ...
  const raw = Array.isArray(category?.subcategories) ? category.subcategories : [];
  const result = [];
  const add = item => {
    if(item == null) return;
    ...
    if(!name) return;
    const id = String(obj.id || obj.key || name).trim();
    if(result.some(x => x.id === id)) return;
    result.push({id,name,image:obj.image?String(obj.image):"",icon:obj.icon||"▦"});
  };
  // المصدر الأساسي للفروع هو parentId.
  categories.filter(c => c.parentId != null && String(c.parentId) === String(categoryId)).forEach(add);
  raw.forEach(add);
  // دعم الصيغة القديمة التي كانت تحفظ اسم الفرع داخل المادة.
  products.filter(p => String(p.cat) === String(categoryId)).forEach(p => {
    const sub = String(p.subcategory || "").trim();
    if(sub && !categories.some(c => String(c.id) === sub)) add({id:sub,name:sub});
  });
  return result;
}
...
function renderCategorySubnav(){
  ...
  if(!nav){
    ...
    nav.id="categorySubnav";
    nav.className="category-subnav";
    document.getElementById("categoriesSection")?.appendChild(nav);
  }
  const title=getCategoryName(selectedCategory)||"القسم";
  const subs=getCategorySubcategories(selectedCategory);
  nav.innerHTML=`
    <div class="category-detail-head">
      <div><span class="category-detail-kicker">تفاصيل القسم</span><h3>${escapeHtml(title)}</h3></div>
      <button type="button" class="category-back" id="categoryBackBtn">‹ الأقسام</button>
    </div>
    <div class="category-subnav-track" role="tablist" aria-label="التصنيفات الفرعية">
      <button type="button" class="category-subtab active" data-subcat=""><span class="category-subtab-icon">▦</span><span>الكل</span></button>
      ${subs.map(sub=>`<button type="button" class="category-subtab" data-subcat="${escapeHtml(sub.id)}"><span class="category-subtab-icon">${sub.image?`<img src="${escapeHtml(resolveStoreImage(sub.image))}" alt="" loading="lazy">`:escapeHtml(sub.icon)}</span><span>${escapeHtml(sub.name)}</span></button>`).join("")}
    </div>`;
  nav.classList.add("show");
  nav.querySelector("#categoryBackBtn")?.addEventListener("click",()=>{
    selectedCategory=categories.find(c => !(c.parentId ?? c.parent_id ?? c.parent))?.id || categories[0]?.id || "";
  ...
  nav.querySelectorAll(".category-subtab").forEach(btn=>btn.addEventListener("click",()=>{
    nav.querySelectorAll(".category-subtab").forEach(x=>x.classList.remove("active"));
    btn.classList.add("active");
    const sub=String(btn.dataset.subcat||"");
    renderProducts(sub);
    document.getElementById("productsTitle")?.replaceChildren(document.createTextNode(sub||title));
...
function renderCategories(){
  ...
  if(!el) return;
  const mainCategories = categories.filter(c => !(c.parentId ?? c.parent_id ?? c.parent));
  el.innerHTML=mainCategories.map(c=>`
    <button class="category ${c.id===selectedCategory?"active":""}" data-cat="${escapeHtml(c.id)}" type="button">
      <div class="category-icon">
        ${getCategoryImage(c)?`<img src="${escapeHtml(getCategoryImage(c))}" alt="${escapeHtml(c.name||"")}" loading="lazy">`:escapeHtml(c.icon||"🛒")}
      </div>
...
function renderProducts(subcategoryFilter=""){
  ...
  if(subcategoryFilter){
    ...
    if(isRealCategory){
      const ids = getCategoryChildrenIds(subcategoryFilter);
      list = products.filter(p => ids.includes(String(p.cat)));
...
function getFilteredProducts(){
  ...
  if(!query){
    ...
    return products.filter(p => ids.includes(String(p.cat)));
  }
