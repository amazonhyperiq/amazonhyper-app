...
function showOfferDetails(o){
 ...
 body.innerHTML=`<span class="offer-details-kicker">عرض خاص</span><h2>${escapeHtml(o?.title||o?.name||'عرض خاص')}</h2><p>${escapeHtml(o?.description||'')}</p><div class="offer-details-items">${items.map(it=>{const p=products.find(x=>String(x.id)===String(it.productId??it.product_id));const img=resolveStoreImage(it.image||p?.image||'');const newP=Number(it.newPrice??it.offerPrice??it.offer_price??p?.price??0);const oldP=Number(it.oldPrice??it.old_price??p?.price??0);return `<div class="offer-details-item">${img?`<img src="${escapeHtml(img)}" alt="" loading="lazy">`:''}<div><b>${escapeHtml(it.name||p?.name||'مادة')}</b><div>${oldP>0&&oldP!==newP?`<del>${money(oldP)}</del> `:''}<strong>${money(newP)} د.ع</strong></div></div></div>`}).join('')}</div>`;
 box.classList.add('show');
}
function renderOffers(){
 const el=document.getElementById('offerCards'); if(!el)return;
 const list=getActiveOffers();
 if(offerBannerTimer){clearTimeout(offerBannerTimer);offerBannerTimer=null;}
 if(!list.length){el.innerHTML='<div style="grid-column:1/-1;text-align:center;padding:24px;color:#6b756f">لا توجد عروض خاصة حالياً.</div>';return;}
 offerBannerIndex=Math.max(0,Math.min(offerBannerIndex,list.length-1));
 ...
 el.innerHTML=`<div class="offer-banner-viewport"><div class="offer-banner-track">${list.map((o,i)=>{
   ...
   const items=Array.isArray(o.items)?o.items:[];
   const itemHtml=items.slice(0,10).map(it=>{const p=products.find(x=>String(x.id)===String(it.productId??it.product_id));const im=resolveStoreImage(it.image||p?.image||'');const np=Number(it.newPrice??it.offerPrice??it.offer_price??p?.price??0);const op=Number(it.oldPrice??it.old_price??p?.price??0);return `<div class="offer-inline-item">${im?`<img src="${escapeHtml(im)}" alt="" loading="lazy">`:''}<div class="offer-inline-info"><b>${escapeHtml(it.name||p?.name||'مادة')}</b><div>${op>0&&op!==np?`<del>${money(op)}</del> `:''}<strong>${money(np)} د.ع</strong></div></div></div>`;}).join('');
   return `<article class="offer-banner-slide" data-index="${i}" data-duration="${Math.max(1,Number(o.duration??o.displaySeconds??7)||7)}"><div class="offer-banner-visual">${img?`<img src="${escapeHtml(img)}" alt="${escapeHtml(o.title||o.name||'عرض')}" loading="lazy">`:'<div class="offer-banner-placeholder">عرض خاص</div>'}</div><div class="offer-banner-copy"><span>عرض خاص</span><h3>${escapeHtml(o.title||o.name||'عرض خاص')}</h3><p>${escapeHtml(o.description||'')}</p>${items.length?`<div class="offer-inline-items">${itemHtml}</div>`:''}<button type="button" class="offer-banner-btn" data-offer-index="${i}">${items.length?'عرض المواد والتفاصيل':'التفاصيل'} ←</button></div></article>`;
 }).join('')}</div></div>${list.length>1?`<button type="button" class="offer-banner-arrow prev" aria-label="السابق">‹</button><button type="button" class="offer-banner-arrow next" aria-label="التالي">›</button><div class="offer-banner-dots">${list.map((_,i)=>`<button type="button" class="offer-banner-dot ${i===offerBannerIndex?'active':''}" data-i="${i}"></button>`).join('')}</div>`:''}`;
 ...
 const schedule=()=>{if(offerBannerTimer)clearTimeout(offerBannerTimer);if(list.length<2)return;const ms=Math.max(1000,Number(list[offerBannerIndex].duration??list[offerBannerIndex].displaySeconds??7)*1000);offerBannerTimer=setTimeout(()=>go(offerBannerIndex+1),ms);};
 el.querySelector('.prev')?.addEventListener('click',()=>go(offerBannerIndex-1));
 el.querySelector('.next')?.addEventListener('click',()=>go(offerBannerIndex+1));
 el.querySelectorAll('.offer-banner-dot').forEach(d=>d.addEventListener('click',()=>go(Number(d.dataset.i))));
 el.querySelectorAll('[data-offer-index]').forEach(b=>b.addEventListener('click',()=>showOfferDetails(list[Number(b.dataset.offerIndex)])));
 show();schedule();
}
/* =====================================================
   تنسيق تاريخ العرض
...
(function injectOfferBannerStyles(){
 ...
 .offer-banner-carousel{position:relative;grid-column:1/-1;padding:0;min-height:230px;max-height:360px;overflow:hidden;border-radius:20px;background:#fff;border:1px solid #dfe9e3;box-shadow:0 10px 28px rgba(5,67,34,.07)}
 .offer-banner-viewport{overflow:hidden;width:100%}.offer-banner-track{display:flex;direction:ltr;transition:transform .65s cubic-bezier(.22,.61,.36,1);will-change:transform}
 .offer-banner-slide{direction:rtl;min-width:100%;display:grid;grid-template-columns:1fr 1.15fr;min-height:230px;max-height:360px;background:linear-gradient(90deg,#f8fbf9,#fff)}
 .offer-banner-visual{min-height:230px;max-height:360px;overflow:hidden;background:#f5f7f6}.offer-banner-visual img{width:100%;height:100%;object-fit:cover;display:block}.offer-banner-placeholder{height:100%;display:grid;place-items:center;font-size:30px;font-weight:900;color:#8254c8;background:#f2eaff}
 .offer-banner-copy{padding:22px 25px;display:flex;flex-direction:column;align-items:flex-start;justify-content:center;min-width:0}.offer-banner-copy>span{background:#f1e9ff;color:#6b3cb1;padding:6px 10px;border-radius:999px;font-size:10px;font-weight:900}.offer-banner-copy h3{margin:9px 0 5px;font-size:23px;color:#15231e}.offer-banner-copy p{margin:0;color:#63716a;line-height:1.65;font-size:13px}.offer-inline-items{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;width:100%;margin-top:10px;max-height:120px;overflow:auto}.offer-inline-item{display:flex;gap:7px;align-items:center;padding:6px;border:1px solid #e2e9e5;border-radius:10px;background:#fff}.offer-inline-item img{width:40px;height:40px;border-radius:8px;object-fit:cover;flex:none}.offer-inline-info{min-width:0}.offer-inline-info b{display:block;font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.offer-inline-info strong{font-size:12px;color:#168a52}.offer-inline-info del{font-size:10px;color:#9a9f9c}.offer-banner-btn{margin-top:11px;border:0;border-radius:11px;padding:9px 15px;background:#8254c8;color:#fff;font-weight:900;cursor:pointer}.offer-banner-arrow{position:absolute;top:50%;transform:translateY(-50%);z-index:3;width:38px;height:38px;border:0;border-radius:50%;background:#fff;color:#333;box-shadow:0 5px 15px #0002;font-size:25px}.offer-banner-arrow.prev{right:12px}.offer-banner-arrow.next{left:12px}.offer-banner-dots{position:absolute;bottom:10px;left:50%;transform:translateX(-50%);display:flex;gap:6px}.offer-banner-dot{width:8px;height:8px;border:0;border-radius:50%;background:#c9d0cc;padding:0}.offer-banner-dot.active{width:22px;border-radius:99px;background:#8254c8}
 @media(max-width:700px){.offer-banner-carousel{min-height:0;max-height:none}.offer-banner-slide{grid-template-columns:1fr;min-height:0;max-height:none}.offer-banner-visual{height:155px;min-height:155px;max-height:155px}.offer-banner-copy{padding:14px 16px 30px}.offer-banner-copy h3{font-size:19px}.offer-inline-items{grid-template-columns:1fr 1fr;max-height:100px}.offer-banner-arrow{width:34px;height:34px}}
