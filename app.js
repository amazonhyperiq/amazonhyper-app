...
function renderCartModal(){
    ...
    lines.push(`
      <div class="cart-line">
        <div>
          ...
          <div class="cart-line-meta">${qty} ${p.unit || "قطعة"}</div>
          <div class="cart-line-price">${money(lineTotal)}</div>
        </div>
        <div class="cart-line-controls">
          <button type="button" onclick="changeQty(${p.id},1); renderCartModal();" aria-label="زيادة">+</button>
          <span>${qty}</span>
          <button type="button" onclick="changeQty(${p.id},-1); renderCartModal();" aria-label="نقصان">−</button>
        </div>
      </div>
    `);
  }
  ...
  itemsEl.innerHTML = lines.length
    ? lines.join("")
    : `<div class="cart-empty">السلة فارغة حالياً 🛒<br>أضف المواد التي تحتاجها ثم ارجع إلى السلة.</div>`;
...
document.addEventListener("DOMContentLoaded", () => {
  // Both cart controls in the header open the same cart; no navigation/refresh.
  document.getElementById("cartOpenBtn")?.addEventListener("click", openCartModal);
  document.getElementById("brandCartBtn")?.addEventListener("click", openCartModal);
